<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Configurações: Impostos') }}</h2>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4 text-right">
                        <a href="{{ route('settings.taxes.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
                            Novo Imposto
                        </a>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($tax->percentage, 2, ',', '.') }}%</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                        <a href="{{ route('settings.taxes.edit', $tax->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">Editar</a>
                                        <form action="{{ route('settings.taxes.destroy', $tax->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Excluir?');">
                                            @csrf @method('DELETE')
                                ...
                                @empty
                                <tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Nenhum imposto cadastrado.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
